...
| 6, exploitation et ouverture | LS-7 | 6 | **Le 23 au soir** : LS-236 (bandeau de réassurance), LS-237 (cause mesurée du faux négatif) et **LS-249** closes, cette dernière corrigeant `/aide` et les CGV qui annonçaient la livraison offerte « quel que soit le mode » ; LS-250, LS-251 et LS-252 ouvertes en chemin, toutes closes depuis. **La recette de l'exploitante du 23 septembre 2026** a produit LS-238 à LS-248, tous clos le jour même, **LS-239** sans correction faute de reproduction : panier borné au stock, ajout depuis la carte, catalogue paginé, sélection multiple de produits et de messages, messages **archivés et jamais effacés** (migration additive `message.archive_a`), photo agrandie, menu d'administration en cinq groupes. **Rien ne se supprime en base de production**, arbitrage de Christophe, LS-246. **LS-153 et LS-142 attendent l'exploitante**. LS-140 et LS-107 attendent les photographies de LS-23. Reste LS-150, la visibilité dans les moteurs de réponse. **LS-228** a livré son gabarit de titre le 13 septembre 2026, sept titres sur quatorze rendaient en police système ; son second volet, les blocs d'état des deux vues d'ensemble, attend LS-153 faute de données sur les comptes de production. **LS-229** a livré le pendant public le 14 septembre 2026 : aucun titre des pages publiques ni des écrans de connexion n'employait le jeton, C42 ne couvrant que les écrans privés. La règle vit désormais dans `globals.css`, hors d'atteinte des sorties anticipées de layout, **C43** l'énonce, et les trois portes d'entrée reçoivent le gabarit à deux panneaux du prototype. **LS-230 est close** le 14 septembre 2026 : les 44 preuves par mutation tournent désormais, 37 par PR et 7 au nocturne. Six étaient cassées et cachaient cinq trous réels, et cinq mutations traînaient dans le dépôt, dont la garde d'accès à l'administration. **LS-231 est close sans correction** le 16 septembre 2026 : un échec sur six exécutions sur `tableau-bord`, jamais observé en CI, défaut connu et accepté. **LS-232 est close** le 17 septembre 2026 : le nocturne du 17 a rendu l'étape de bout en bout verte, 2155 tests passés contre douze échecs le 15, sans qu'aucun test disparaisse ni soit ignoré. **LS-233 a reçu un second correctif** le 17 septembre, et elle est close le 25, son premier ayant régressé : le filtre ancré sur `× ` et `✘ ` ne retenait plus rien sous les reporters de CI, Playwright passant à `github` et Vitest encodant la virgule en `%2C`. Le filtre lit désormais les quatre formes mesurées dans les deux modes. Le même diagnostic a montré que le step groupant six preuves sous `bash -e` en masquait quatre depuis le 14 septembre. **LS-234 est close** le 16 septembre, `robots.txt` déduit son état du catalogue. **LS-223, LS-226 et LS-227 sont closes** le 13 septembre 2026 : la clé Backblaze durcie, l'instabilité des tests et le cul-de-sac de la réauthentification. **LS-235 est close** le 24 septembre 2026. Le 22 : le nocturne annulé trois nuits sans alerte, borne locale posée et prouvée, plafond porté à 75 min, `npm audit` remonté hors d'atteinte, et l'alerte déplacée dans `veilleur-nocturne.yml`, **une étape interne ne pouvant pas signaler le dépassement du plafond de son propre job**. Le nocturne du 21 a été le premier à tourner avec la borne : **elle a coupé, et c'est sa réussite**, l'étape s'est nommée. Ce qu'elle a montré a déplacé le diagnostic, **le coût était la cause et non le plafond** : chacun des 147 cas d'intégration relançait la suite entière, 419 s pour 947 tests, soit 17 h 06. Le script cible désormais le fichier qui porte la garantie, 2377 s mesurées, et **cinq défauts préexistants** sont tombés au premier passage allant au-delà du cas 1. Le septième critère, le nocturne au vert, est rempli le 24. **Le 24 septembre 2026, avec LS-252**, les nuits des 22 et 23 ont nommé la cause restante : les dix cas de bout en bout rejouaient chacun toute la suite Playwright, 11 min pour le premier sur le runner. Ciblés, puis six corrections de cas périmés ou aveugles, dont un **oracle** sans test sur le remboursement de rétractation : première exécution complète, **180 mutations en 3507 s** en local. La borne de l'étape passe à 150 min et le plafond du job à 180, arbitrage de Christophe, à resserrer sur la durée par cas que le script imprime désormais. **LS-253 est close** le même jour, le catalogue d'administration débordait de 35 px à 320 px sous la police du runner Linux. **LS-251 est livrée** le 24 septembre : la réassurance sur la fiche (bloc 7), le panier et le tunnel, un sous-ensemble du même composant par écran. **LS-252 et LS-235 sont closes** le même soir : le nocturne est vert, `180 mutations, 180 detectees`, et l'étape des preuves passe de 2 h 05 à 1 h 22 depuis que chaque cas ne lance que son test attendu, bornes réglées sur cette mesure à 165 et 200 min. **LS-254** ajoute le contrôle à sec des expressions de mutation à chaque PR. **LS-250 est close** le 25 septembre : plus aucun tube vers `grep -q` sous `pipefail`, liste des restes supprimée. **LS-237** en est sorti, un faux négatif intermittent du cas 12 |
...
**224 tickets terminés sur 244 hors epics**, les deux termes relevés dans Jira
le **25 septembre 2026** au matin, et jamais dérivés l'un de l'autre : 20
tickets non terminés relevés séparément, 9 En cours et 11 À faire, et 244 − 20
retombe bien sur 224. **La recette de l'exploitante du 23 septembre** a ouvert
LS-238 à LS-248, tous clos le jour même.

**Le chantier du nocturne est clos le 24 septembre 2026** : LS-233, LS-235,
LS-250, LS-252, LS-253 et LS-254, avec LS-251 livrée et déployée. Le nocturne est
vert deux fois de suite, `180 mutations, 180 detectees`, étape des preuves en
1 h 22. Journaux `2026-09-24-a` à `-e` et `2026-09-25-a`.

**Aucun ticket ouvert ne dépend plus seulement du code.** Les neuf En cours
attendent l'exploitante, une démarche externe ou l'ouverture commerciale :
...
**LS-237 est close** le 23 septembre 2026, cause mesurée : un
`printf | grep -q` sous `pipefail`, que SIGPIPE faisait échouer sur un motif
pourtant trouvé, 17 fois sur 200. La même forme produisait le faux positif
transitoire de `verifier-config-claude.sh`. `verifier-grep-q-pipefail.sh`
l'interdit désormais ; **LS-250** a converti les 41 occurrences restantes le
25 septembre 2026, et **LS-252** a fermé l'arrêt de la preuve complète.
...
**Onze des vingt tickets ouverts attendent l'exploitante** : les dix de
l'epic LS-22, photographies, textes, tarifs et contenus juridiques, et LS-19,
la médiation, qui suppose une adhésion à souscrire. Aucun code ne les débloque.
Comptes relevés dans Jira le 25 septembre 2026, `parent = LS-22 AND
statusCategory != Done` en rendant dix : une formule antérieure en annonçait
...
`verifier-tests-mutation.sh` casse **180 fois** au 17 septembre 2026 le comportement
