...
|---|---|---|
| 2, catalogue, médias et stock multicanal | LS-3 | en cours, **sept stories ouvertes**, relevé dans Jira le 8 septembre 2026 après la fermeture de LS-166, LS-163 et LS-193. **LS-193 LIVRÉE ET CLOSE le 8 septembre 2026**, PR #299 : le logo écrit « Lune-soleil », le code portait « Lune & Soleil », et l'image de partage montrait les DEUX graphies dans la même image. Douze fichiers l'écrivaient en dur, dont `layout.tsx` qui importait déjà `NOM_BOUTIQUE` pour son gabarit tout en recopiant le titre par défaut trois lignes plus haut. **Deux contrôles étaient verts sur des défauts réels** : `verifier-seo.sh` cherchait l'ancienne graphie en dur, donc il est devenu **aveugle en restant vert** au changement ; et mon propre contrôle a laissé passer quatre occurrences visibles, dont l'en-tête de toutes les pages publiques, parce que **trois formes existent** et qu'il n'en cherchait qu'une, la forme JSX `&amp;` et la forme coupée par l'enveloppement à 80 colonnes lui échappant. **Mes quatre mutations n'avaient rien prouvé**, injectant toutes la même forme : une mutation prouve que le contrôle voit ce qu'elle fabrique, jamais ce qui existe déjà. Six mutations sur six après correction. La dénomination sociale n'était pas atteignable, `FACTURE_RAISON_SOCIALE` venant de l'environnement. **LS-163 LIVRÉE ET CLOSE le 8 septembre 2026**, PR #298 : trois listes plafonnaient à 100 sans le dire, et l'écran des messages comptait **sur la tranche tronquée**, donc il aurait annoncé « 100 messages » pour toujours une fois le seuil franchi, un `NOUVEAU` plus ancien devenant invisible ET non compté. Arbitrage : signaler et filtrer, jamais paginer, la pagination coûtant une barre à 320 px pour un seuil lointain. Quatre écrans traités, chacun avec sa raison, dont les rétractations hors ticket où **l'original avait divergé de ses copies**. **Trois formes de comptage mesurées** : `groupBy` un échec sur trois, deux `count` en parallèle trois sur dix donc pire, une requête `FILTER` dix verts trois fois. La cause n'était pas le volume, six messages en base, mais le MOMENT, ce comptage s'exécutant pendant la revalidation de layout de C37. **LS-166 LIVRÉE ET CLOSE le 8 septembre 2026**, PR #296 : la quatrième largeur de l'invariant 10, 768 px, n'était mesurée par aucun test depuis le début du projet. **Aucun défaut de mise en page ne lui est propre**, et ce qu'elle a révélé est ailleurs : les liens de l'administration préchargeaient **en boucle sans fin**, `staleTimes.dynamic` valant zéro sur des routes `force-dynamic`, et cette boucle affamait parfois la navigation réelle au point de la **bloquer définitivement**, mesuré à 61 secondes. Règle **C40** posée. Durée de 2 min 07 s à 2 min 40 s pour 1106 puis 1466 tests. **LS-198 LIVRÉE ET CLOSE le 7 septembre 2026**, PR #287 : la fiche produit sert **neuf URL de média**, mesurées, dont deux vignettes `320.jpeg` qu'aucun test ne confrontait à un code de statut. **Deux affirmations du ticket ont été démenties par la mesure** : la galerie n'est pas le seul écran à servir du 1920 px, `LARGEURS_SRCSET` filtrant à 1280 et **aucun écran** ne le référençant, ce qui était l'argument principal en faveur d'un second test. Le trou réel est ailleurs, `320.jpeg` étant servi par trois écrans. Un test **générique** parcourt une table d'écrans plutôt que de recopier celui du catalogue, laissé hors table car il fige un compte exact plus strict. La mutation a rencontré un piège : retirer le fichier de `public/medias/` laisse le test **vert**, le `webServer` rejouant la préparation qui le recrée ; la cible réelle est `.next/standalone/public/`. **LS-143 LIVRÉE ET CLOSE le 7 septembre 2026**, PR #276 : `formaterOrigine` portait un `switch` à `default` rendant « Système » pour toute valeur inconnue, donc une quatrième valeur d'`OrigineEcriture` se serait affichée **faussement** comme une écriture système. **La cause n'était pas le `default`** mais l'élargissement en amont, le service déclarant `origine: string` et jetant l'enum que Prisma rend : la correction porte sur deux endroits. `LIBELLES_PAIEMENT` était le **jumeau exact**, fermé aussi. Contre-preuve faite : avec l'ancien code, l'enum à quatre valeurs laissait le `type-check` **vert**. Le module n'avait **aucun test** alors que dix fichiers l'importent, ce que le critère 5 supposait acquis : treize tests ajoutés, deux mutations les éprouvent. **LS-161 LIVRÉE ET CLOSE le 7 septembre 2026**, PR #277 : un `aria-describedby` visait une région portant `aria-label`, et le calcul de la description consultant le label avant le contenu, le bouton s'annonçait « Générer le document, Génération du document » **jamais** le message réel. **La justification écrite était fausse** et c'est elle qui a propagé le défaut sur un second écran, LS-160 n'en ayant corrigé qu'un. Règle **C39** posée, cinq mutations sur cinq, dont un cas qui garde le contrôle contre lui-même : un `aria-label` cité **en commentaire** ne doit pas faire rougir. Relevé du 6 septembre 2026 APRÈS la fermeture de LS-196. **LS-196 LIVRÉE le 6 septembre 2026** : le lien d'évitement de la boutique mesurait **25,59 px de haut pour 44 exigés** et revenait à `left: 0`, ce qui coupe son contour de focus, WCAG 2.4.7. **Étendre le contrôle à la boutique a trouvé trois écrans qui rendaient un `<main>` nu**, deux états d'erreur et un état de chargement : le lien y menait nulle part, défaut de LS-191 rejoué côté public. Un quatrième est venu de la revue, `not-found.tsx` composant `EnTeteBoutique` hors du groupe de routes ; la recherche se fait désormais par **ce qui rend l'en-tête** et non par l'arborescence. 54 écrans couverts. **J'ai perdu ma propre correction** en lançant le script de mutation avant de commiter, sa restauration `git checkout HEAD` rendant la version d'avant le travail : le contrôle livré échouait sur son propre dépôt. **Sa preuve par mutation annonçait « 8 sur 8 » sur un contrôle rouge**, `jouer()` n'exigeant qu'un code non nul et un motif présent, deux conditions que le défaut préexistant remplissait déjà ; deux gardes ajoutées et éprouvées, état de référence vert exigé et empreinte des fichiers mutables. **Une raison écrite était fausse**, le `z-index` justifié par un contexte d'empilement qui n'existe pas. C34 gagne les deux seuils qui manquaient. **LS-199 créée**, Haute : le contrôle nocturne échoue depuis deux nuits sans que personne ne le voie, un de ses tests étant périmé par LS-185. **LS-108 LIVRÉE le 6 septembre 2026** : `--ls-border` bordait les champs de saisie à **1,57:1 sur blanc et 1,31:1 sur sable**, quand WCAG 1.4.11 exige 3:1 pour la limite d'un contrôle que rien d'autre n'identifie, le fond du champ et celui de sa carte étant tous deux blancs. **LS-108 LIVRÉE le 6 septembre 2026** : `--ls-border` bordait les champs de saisie à **1,57:1 sur blanc et 1,31:1 sur sable**, quand WCAG 1.4.11 exige 3:1 pour la limite d'un contrôle que rien d'autre n'identifie, le fond du champ et celui de sa carte étant tous deux blancs. La palette gagne `--ls-border-controle` `#95836a`, sur **trente-cinq bordures** : c'est le **sable qui a dicté la valeur**, deux candidats plus clairs passant sur blanc en échouant sur sable, ce qui aurait recréé le piège de C31. Le contrôle s'est trompé **dans les deux sens** avant de mesurer une paire : chercher `var(--ls-border)` laissait passer trois champs bordés de `--ls-text-muted`, contournement conforme mais invisible ; exiger le jeton dédié **dégradait** treize boutons primaires bordés de `--ls-primary` à 8,93:1. La revue a trouvé **quatre chiffres faux, tous recopiés sans être relevés**, dont « huit écrans » pour vingt-cinq, et un **angle mort** fermé dans la session : un bloc `:hover` isolé ne porte ni nom d'élément ni marque tactile, forme exacte de la rechute. Neuf mutations sur neuf, et 44 bordures mesurées sur le rendu réel à 320, 390, 768 et 1280 px, hors Playwright dont les projets n'en couvrent que trois, sujet de LS-166. LS-198 créée le même jour. **LS-187 et LS-192 LIVRÉES le 6 septembre 2026**, PR #257 : les cinq écrans de la boutique collaient le nom de fichier au chemin du média, ce qui rend **404** sur toute photographie réelle, et non le 308 que le ticket annonçait. **LS-187 accusait l'éditeur de produit, qui était le seul écran correct du dépôt** : sa description a été réécrite. La cause est une **fixture de test** qui posait `produits/e2e-ls104/` en affirmant que c'était la forme du traitement, alors que `exigerSegmentSimple` refuse le slash : la barre réparait la faute au passage, donc aucun test ne pouvait la voir. `urls.ts` porte désormais la construction pour les sept écrans, `declinaisons.ts` sort les constantes d'ADR-007 du module qui importe sharp, sans quoi le panier, composant client, tirait un binaire natif dans son bundle. La revue a trouvé que le test des sept URL **passait en local et aurait rougi en CI**, `public/medias/` étant ignoré par git : un générateur tourne maintenant avant le build, `next build` recopiant `public/`. **LS-195 LIVRÉE le 6 septembre 2026**, PR #256 : le catalogue public écrivait « Chargement des pièces. » avec un point final, seul des quinze annonces du dépôt, alors qu'il était la **référence** citée par le composant partagé. Les quatorze copies de LS-188 ont corrigé la forme au passage sans que l'original le soit : c'est l'original qui a divergé de ses copies, ce qu'aucune relecture de diff ne montre. La règle **C35** posée par la story affirmait « comme partout ailleurs dans le dépôt », et la revue a mesuré que **six libellés de bouton la démentaient**, dont deux sur le parcours de rétractation : ils sont alignés avec elle plutôt que de publier une règle que son propre dépôt contredit. Le contrôle a **deux sens**, sa première version n'ayant pas de dénominateur : reformuler une annonce faisait passer le compte de 15 à 14 en silence et en vert. **LS-194 LIVRÉE le 6 septembre 2026**, PR #255. **LS-195 LIVRÉE le 6 septembre 2026**, PR #256 : le catalogue public écrivait « Chargement des pièces. » avec un point final, seul des quinze annonces du dépôt, alors qu'il était la **référence** citée par le composant partagé. Les quatorze copies de LS-188 ont corrigé la forme au passage sans que l'original le soit : c'est l'original qui a divergé de ses copies, ce qu'aucune relecture de diff ne montre. La règle **C35** posée par la story affirmait « comme partout ailleurs dans le dépôt », et la revue a mesuré que **six libellés de bouton la démentaient**, dont deux sur le parcours de rétractation : ils sont alignés avec elle plutôt que de publier une règle que son propre dépôt contredit. Le contrôle a **deux sens**, sa première version n'ayant pas de dénominateur : reformuler une annonce faisait passer le compte de 15 à 14 en silence et en vert. **LS-194 LIVRÉE le 6 septembre 2026**, PR #255. **LS-194 LIVRÉE le 6 septembre 2026**, PR #255 : les seize écrans d'administration n'avaient aucun lien d'évitement, WCAG 2.4.1 niveau A, et atteindre le contenu au clavier demandait de traverser les onze rubriques de la barre à chaque changement d'écran. Le lien vit dans le layout sous le retour anticipé hors rôle, la cible dans le `<main>` de chaque page : la description recommandait l'inverse, un conteneur posé par le layout, que **LS-122 avait déjà écarté en le mesurant**, `focus()` sur un `div` sans `tabindex` ne prenant pas. `verifier-lien-evitement.sh` entre en CI, six mutations sur six. La revue a trouvé que le contrôle acceptait l'ancre posée **hors** du `<main>`, et que le contour de focus sortait du cadre à `left: 0`, l'assertion `x >= 0` laissant passer ce cas. **LS-196 créée** : le lien de la boutique porte les mêmes deux défauts. **LS-113 LIVRÉE le 6 septembre 2026**, PR #252 : la suite couvrait ZÉRO des cinq états obligatoires de `frontend-design.md` sur ces écrans, et ne cliquait sur **aucun bouton d'administration**, donc ni l'état pending, ni le double clic sur une Server Action, ni le message de refus n'étaient exercés. Cinq états vides sont rendus et assertés, chacun avec son cas négatif. Deux d'entre eux sont **inatteignables de bout en bout**, l'état dépendant d'une table globalement vide : ils passent par un **troisième projet Vitest en jsdom**, arbitrage de Christophe, `npm run test:composant`. Elle a corrigé **LS-168 à sa source**, parce que le plafond empêchait de prouver son propre travail : la fixture cliente s'inscrivait à chaque exécution avec une adresse horodatée quand `/sign-up/email` n'accepte que trois appels par minute. Trois exécutions d'affilée passent désormais sans 429, et un test de navigation qui échouait sur `main` disparaît avec. **LS-188 LIVRÉE le 5 septembre 2026**, PR #250 : quinze écrans déclaraient `force-dynamic` sans aucun état de chargement, chaque navigation attendant le rendu serveur complet sans que rien ne bouge à l'écran. Dix reçoivent un `loading.tsx` s'appuyant sur un composant partagé, cinq passent au `<Suspense>` interne, un est dispensé faute d'attente réelle. Elle a fermé un défaut de correction introduit en cours de route et **révélé trois trous préexistants** : un `loading.tsx` posé sur un segment racine fait passer en 200 les 404 de tout son sous-arbre, `verifier-loading-et-404.sh` ne comparait que des fichiers voisins, la règle C32 énonçait la version étroite, et les deux écrans de détail n'avaient **aucun** test de code de statut alors que le critère 5 le supposait acquis. **LS-195 créée** par la revue : le catalogue public écrit « Chargement des pièces. » quand les quinze autres emploient des points de suspension. **LS-147 LIVRÉE ce jour**, PR #245 : le site n'avait aucun favicon, aucun manifeste et aucune image de partage. Elle a découvert un défaut plus large qu'elle, hérité de LS-137 : **zéro `og:image` sur les six pages publiques**, les balises Open Graph ayant été posées sur 25 pages sans jamais fournir d'image. La convention `opengraph-image.tsx` de Next.js **n'a pas tenu**, servant la balise en développement et la perdant sous `next start` ; l'image passe donc par un PNG statique en URL absolue, arbitrage de Christophe. **LS-193 créée** : le logo écrit « Lune-soleil » quand le code porte « Lune & Soleil », 24 fichiers dont les mentions légales et les emails. **LS-192 créée** ce jour, Low : le préfixe des médias vit sous deux noms de variable différents dans cinq fichiers, aucun dans `.env.example`. **LS-181 créée ET LIVRÉE ce jour**, PR #228 : la barre devient latérale et repliable, les pastilles de comptage viennent des données, le tableau de bord existe (il tenait en un `h1` depuis LS-70), le tableau d'expédition passe à trois colonnes, et la **déconnexion, absente depuis LS-70**, est livrée. Trois écarts au prototype sont assumés et mesurés, le fond de barre en tête : `--ls-text-muted` sur sable donne 4,35:1, sous le seuil AA. **LS-182 créée puis DÉCOUPÉE** le même jour en LS-183, LS-184 et LS-185 : trois écrans du prototype n'avaient aucun ticket, le métier existant sans son écran. **LS-183 livrée en session F** : l'écran Catalogue, qui rendait l'éditeur inatteignable sans connaître un UUID. **LS-187 créée** : l'éditeur ajoute une barre en trop au chemin des vignettes, 308 au lieu de 200, chaque image passant par une redirection. **LS-191 LIVRÉE le 5 septembre 2026**, PR #247 : quinze des seize écrans d'administration n'avaient aucun `error.tsx`, donc une erreur remontait à `global-error.tsx`, qui remplace le layout racine et fait perdre la barre de navigation. Un seul `error.tsx` couvre les seize, avec réessai, identifiant de corrélation et journalisation serveur par `instrumentation.ts`. Son critère 6 a demandé un arbitrage : aucun chemin ne permettait de faire lever un écran depuis l'extérieur, d'où une route qui lève à dessein, **introuvable en production**, gardée par trois contrôles. **LS-127 fermée** le même jour, reprise par LS-191 et LS-188. **LS-194 créée** par la revue : l'administration n'a aucun lien d'évitement sur ses seize écrans. LS-84, LS-146 et LS-162 livrées le 2 septembre ; **LS-165** et **LS-166** créées le même jour par les revues. **LS-171 créée et close le 3 septembre** : un libellé sans espace débordait de 43 px à 320 px sur le détail de commande |
...
| 4, factures, avoirs, emails, expédition et contact | LS-5 | **presque terminée** : **quatorze stories closes et cinq ouvertes**, relevé dans Jira le 6 septembre 2026. **LS-200 créée le 6 septembre**, arbitrage de Christophe : le raccordement de l'API **Sendcloud**, qui remplace l'API Mondial Relay visée par ADR-025 depuis l'ouverture du compte, ADR-035. Elle **bloque LS-131**, le contrat `FournisseurPointsRetrait` et sa dégradation étant déjà livrés par LS-115 : seul l'appel HTTP manque. État d'avant, relevé le 5 septembre 2026, dont **LS-184 le 5 septembre 2026**, PR #234, l'écran Factures et avoirs. Retrouver une facture depuis son numéro était impossible, le seul chemin passant par le détail d'une commande. Les deux tables sont fusionnées en une liste chronologique, le montant d'un avoir étant rendu négatif à la lecture pour qu'aucun appelant ne le compte comme une recette. Onze autres closes, dont LS-130, LS-97, LS-154 et LS-82 le 2 septembre. Quatre ouvertes, dont **trois attendent une démarche et non du code** : LS-33 ne porte plus que la souscription Mondial Relay, sa décision technique étant tranchée depuis le 28 juillet 2026, et LS-131 attend les accès de cette même API. **LS-172 livrée le 3 septembre 2026** : l'email de confirmation porte les deux liens signés, facture et rétractation, qui étaient émis puis perdus depuis LS-132 |
| 4bis, espace client, avis et carnet d'adresses | LS-36 | **quatre stories ouvertes**, relevé dans Jira le 7 septembre 2026 après la fermeture de LS-179. **LS-190 est À MOITIÉ LIVRÉE et reste EN COURS**, PR #293 : le panneau « Documents et actions » regroupe la facture, la rétractation et **le contact qui ne figurait pas du tout** sur l'écran de détail de commande. Fond mesuré sur l'élément **rendu**, 8,93:1, et `--ls-focus` y valant **1,45:1** le contour de focus est passé au blanc, défaut qu'aucun contrôle ne voit puisque `verifier-contraste.sh` mesure les paires texte et fond, pas les indicateurs de focus. **La story dégradait la navigation qu'elle améliorait** : en fusionnant trois sections elle retirait deux `h2` de la liste des titres sans les rendre, corrigé par trois `h3`. La revue frontend a trouvé trois défauts réels, dont un test qui n'exerçait que le tiers du panneau, la fixture n'écrivant pas `statut`. **La frise d'étapes reste à faire**, bloquée par LS-58 : elle suppose un suivi que rien ne renseigne. **LS-179 LIVRÉE ET CLOSE le 7 septembre 2026**, PR #272 : les champs de mot de passe n'offraient aucun moyen de relire la saisie, alors qu'ADR-023 impose seize caractères. Le ticket listait **quatre écrans, le dépôt en portait six sur cinq fichiers** : les deux champs du profil manquaient à sa liste, dont « Nouveau mot de passe », qui est le cas d'usage le plus fort de la story. Arbitrage de Christophe de les inclure. Le profil est **non contrôlé**, il lit son `FormData` et vide ses champs par `reset()` : le forcer en contrôlé aurait fait survivre le mot de passe à un changement réussi, sur un poste possiblement partagé, d'où un composant qui accepte les deux formes. Sept mutations sur sept, dont **deux ont accusé les tests eux-mêmes** : un second `<input>` reste invisible à `getByLabelText`, qui cible par le `htmlFor`, et une assertion de non-fuite visant la seule région live prouvait « la valeur n'est pas là » en énonçant « la valeur n'est nulle part ». `verifier-bascule-mot-de-passe.sh` entre en CI, cinq sens prouvés, et **le contrôle portait le défaut qu'il cherchait** : il trouvait « Afficher le mot de passe » dans un commentaire du composant et restait vert sur un code ayant perdu le libellé, troisième forme du motif « contrôle satisfait par un commentaire ». Mesures à 320 px : bouton 67 x 44 px, bord droit à 303 sur 320, champ à 219 px. **LS-180 livrée**, PR #233 : l'espace client a enfin un `layout.tsx` et une barre latérale, absents jusque-là, et son critère 4 n'est rempli qu'à moitié, la frise d'étapes supposant un suivi que LS-33 bloque, d'où **LS-190 créée**. **LS-185 livrée**, PR #235 : l'écran Clients, dont l'arbitrage RGPD a précédé le code et dont la recherche libre est un **écart assumé à ADR-027**, tracé à quatre endroits pour ne pas être pris pour un oubli. État d'avant, relevé le 4 septembre 2026 : Cet epic était annoncé « au bout de ce qui est faisable » le 3 septembre, et **deux tickets d'interface l'ont rouvert le 4** : **LS-180**, l'espace client ne ressemble pas au prototype faute de tout gabarit partagé, aucun `layout.tsx` n'existant sous `/compte/` ; **LS-179**, aucun moyen de relire un mot de passe de seize caractères minimum. Les stories fonctionnelles restent closes : LS-54 à LS-57, LS-59, LS-60, LS-62, LS-164 et LS-167 closes, soit le socle d'authentification, le rattachement, l'historique et les factures, le carnet d'adresses, le profil, la réauthentification client et les droits RGPD. **Trois ouvertes**, LS-58, LS-61 et LS-77. Seule LS-58 dépend réellement du compte Mondial Relay : **LS-61 et LS-77 ne portent aucun lien de blocage**, vérifié dans Jira le 3 septembre 2026 contre une affirmation périmée de ce tableau. LS-61 bute en pratique sur la date de livraison que LS-131 renseignera, l'invitation partant après livraison |
| 5, rétractation, conformité et protection des données | LS-6 | **une story ouverte**, relevé dans Jira le 8 septembre 2026 après la fermeture de LS-203. **LS-203 LIVRÉE ET CLOSE le 8 septembre 2026**, PR #307 : `traitement-demande.tsx` était le **seul** fichier de l'administration sans aucune annonce d'attente, quand quinze existent dans quatorze fichiers. Les six boutons l'annoncent désormais, **chacun nommant son action**, et la région live aussi : un lecteur d'écran ne vocalise pas le libellé d'un bouton sans focus, donc l'annonce visuelle seule n'aurait servi qu'à la vue. **Deux défauts trouvés en écrivant le test.** Le libellé du constat contenait la séquence **littérale** `\u2019` au lieu d'une apostrophe, donc le bouton affichait « Enregistrer l\u2019état de la pièce » : ni `tsc` ni le lint ne pouvaient le voir, la chaîne étant valide, et seul un `getByRole` sur le nom accessible l'a trouvé. Et **le test consommait ce qu'il mesurait** : ralentir la Server Action faisait écrire le constat, irréversible par conception, donc le premier des trois projets de largeur consommait le geste et les deux autres ne trouvaient plus le bloc, tous passant pourtant en isolation. `route.abort()` ferme le cas sans rien écrire, donc sans nettoyage à oublier. Le débordement est mesuré **pendant** l'attente, plusieurs libellés étant plus longs que celui qu'ils remplacent. 1503 tests de bout en bout au vert. **LS-173 LIVRÉE ET CLOSE le 8 septembre 2026**, PR #305. **LS-173 LIVRÉE ET CLOSE le 8 septembre 2026**, PR #305 : l'étape 9 du parcours 5 n'avait **aucun chemin**, une pièce revenue intacte restant sortie du stock indéfiniment. L'exploitante constate désormais l'état réel de la pièce, avec son motif : `REMISE_EN_VENTE` écrit un mouvement `RETOUR` compensant la vente web, `PERTE_CONSTATEE` n'en écrit **aucun** et pose quand même un état, sans quoi une pièce perdue serait indistinguable d'une demande jamais traitée. Contrainte **C41** en équivalence, qui ne dit **rien de `recueA`** : une pièce jamais revenue se déclare perdue, règle L13, et lier les deux fermerait le seul geste qui solde un colis perdu en transit. **Trois défauts réels trouvés en route, deux introduits par la story elle-même.** `findFirst` ne réintégrait **qu'une pièce sur deux**, le webhook écrivant un mouvement par ligne : le défaut que la story ferme, reproduit à l'intérieur d'elle, et silencieux sur le cas à un article que tous ses tests utilisaient. Une **variante archivée** faisait diverger le journal de la colonne, `incrementerStockPhysique` filtrant sur `archivee_a IS NULL` et rendant un compte de lignes ignoré : le journal totalisait zéro pendant que la colonne disait zéro aussi, et le geste était **brûlé**, l'index de compensation consommé. Le troisième venait de `ls-frontend-revue`, `etatConstateA` **lu en base puis jeté**. Huit mutations sur huit, C41 éprouvée sur ses quatre combinaisons. **Elle a rendu instable un test voisin**, sur les catégories : le bloc étant rendu une fois par demande, la neutralisation a isolé la cause et le repli en `<details>` l'a fermée. **LS-203 créée** : les six boutons de cet écran n'annoncent pas leur attente, dette préexistante. **LS-133, LS-134 et LS-135 livrées le 3 septembre 2026** : le calcul du délai, la fonctionnalité en ligne de l'article L221-21, puis la face administration, étapes 6 à 8. **LS-136 LIVRÉE le 5 septembre 2026**, PR #242 : le tunnel ne portait **aucune** occurrence du mot « rétractation », alors que l'article L221-23 impose l'information avant la validation et que L221-20 sanctionne son absence par un délai porté à douze mois. Une source unique alimente les trois emplacements, et deux revues y ont trouvé quatre défauts réels, dont **deux composants de formulaire là où j'en voyais un**, celui du chemin sans compte portant sa propre copie du texte. **Trois stories ouvertes**, relevé dans Jira le 6 septembre 2026 : LS-148 bloquée par LS-141, plus **LS-173 et LS-174 créées le 3 septembre**, l'étape 9 de réintégration de stock sans chemin et le numéro d'avoir invisible après rechargement |
| 6, durcissement, exploitation et ouverture | LS-7 | **LS-204 LIVRÉE ET CLOSE le 8 septembre 2026**, PR #310 : le contrôle `9a bis` avait bloqué une pull request **documentaire** en affichant « configuration Claude Code cohérente » **sous** le mot ÉCHEC. **La cause de l'intermittence n'est PAS identifiée**, et le ticket est fermé en le disant : quatre mesures ne l'ont pas reproduite, 25 exécutions du contrôle, 6 du script complet dans un clone superficiel imitant `fetch-depth: 1`, le contrôle avec le témoin du cas 4 présent, et le contrôle seul dans ce clone. Ce qui est corrigé est le défaut **établi** : `attendre_vert` lançait le contrôle **deux fois**, une pour le code et une pour l'affichage, donc le rapport décrivait une exécution en concluant sur une autre ; `attendre_rouge` portait le défaut symétrique. L'en-tête du script porte cette limite, la prochaine occurrence ne repartira pas de zéro. **LS-175 EN COURS**, PR #310 : la part sans VPS est livrée, six critères sur dix. Le script d'amorçage rétrograde et promeut dans **une seule transaction**, l'index partiel E1 n'admettant qu'une administratrice et deux instructions séparées laissant une fenêtre où l'administration devient inaccessible ; il refuse une adresse inexistante plutôt que de rendre un succès silencieux, et vérifie par **relecture** et non par `rowCount`. Six cas éprouvés. `verifier-comptes-production.sh` porte le critère 10, trois sens éprouvés sur base jetable. **Le test d'accès croisé du critère 8 existait déjà** depuis LS-70, le commentaire du ticket le disait sans porteur : vérifier avant de conclure a évité de réécrire un test existant. **Quatre critères restent ouverts** et exigent la présence de l'exploitante et le domaine de production, le `rpID` d'une passkey étant le domaine. découpée le 27 août 2026, **onze stories ouvertes**, relevé dans Jira le 8 septembre 2026 après la fermeture de LS-189. **LS-189 LIVRÉE ET CLOSE le 8 septembre 2026**, PR #303 : la suite de bout en bout tourne désormais sur **sa propre base**, port 55433, `npm run db:e2e`. La préparation promeut son compte d'administration et l'index partiel n'admet **qu'une** administratrice, règle E1 : le compte réel de l'exploitante occupait cette place, et la suite entière était marquée « did not run ». **Élargir la clause `e2e-` aurait fermé ce défaut en ouvrant celui qu'elle empêche**, un `UPDATE` sans clause retirant son rôle au compte réel en silence ; l'isolement par la base ferme les deux, et le fait structurellement. **Trois défauts sont sortis de la confrontation à la condition réelle**, aucun visible à l'écriture : le bloc `env` de `webServer` ne gouverne que le sous-processus du serveur, donc les cinq `.setup.ts` écrivaient encore sur la base de développement, et **c'est la base de test restée VIDE qui a désigné la cause** ; un repli en chaîne vide **écrasait** la variable héritée au lieu de laisser le repli jouer, faisant échouer le build sur « DATABASE_URL absente » ; et le garde-fou du script **comparait la variable à elle-même**, Playwright l'ayant déjà surchargée, son refus étant juste dans son principe et faux dans son ancrage. Critères 2 et 3 tenus par mesure, `mis_a_jour_a` identique à la milliseconde après deux exécutions. Quatre mutations sur quatre, chacune sur un état réellement existant, et **la mutation 1 a d'abord raté sa cible** après réécriture de la surcharge : la garde d'empreinte l'a signalé au lieu de rendre un vert trompeur, première fois qu'elle sert sur ce dépôt. Contrôle **9x** en CI. Relevé après la fermeture de LS-202 et LS-197. **LS-197 LIVRÉE ET CLOSE le 7 septembre 2026**, PR #291 : `NEXT_PUBLIC_MEDIA_PREFIXE` est substituée par `next build` et le Dockerfile ne la portait ni en `ARG` ni en `ENV`, donc tout `docker build` figeait `/medias` et aucune configuration d'exécution ne pouvait le changer. **La troisième voie du ticket a été écartée sur mesure** : Nginx ne porte aucun bloc `/medias` et aucun volume n'est monté, donc figer une constante aurait décidé à la place de LS-138. Prouvé sur l'image réellement construite, `--build-arg` faisant atteindre `/cdn-preuve` au bundle. **Deux formes sont normales dans le bundle**, inlinée si la variable est définie, expression entière sinon : ma première version du contrôle cherchait la valeur entre guillemets et n'en trouvait aucune, elle aurait été un faux négatif permanent. Cinq mutations sur cinq. **LS-202 LIVRÉE ET CLOSE le 7 septembre 2026**, PR #281 : six numéros d'étape étaient portés par deux étapes de `controles.yml`, donc « échec à l'étape 6o » désignait deux domaines sans rapport. **La renumérotation demandée ne rentrait pas**, 28 étapes pour 25 lettres, et la cause était un **classement** : quatre étapes seulement validaient le schéma, les vingt-six autres étant des contrôles textuels numérotés `6x` par accumulation. Un **contrôle 9** leur est détaché, arbitrage de Christophe, et une preuve par mutation porte le numéro de ce qu'elle prouve, suffixé `bis`. **Le nom du job n'est PAS renommé** : « Les huit controles de CONTRIBUTING » décrit désormais neuf contrôles donc paraît faux, mais c'est l'**identifiant** exigé par la protection de branche, vérifié à l'API ; le corriger laisserait toute pull request en attente d'un résultat qui n'arrive jamais. Cinq mutations sur cinq, dont deux gardent le contrôle contre lui-même. Relevé après la fermeture de LS-156. **LS-156 LIVRÉE ET CLOSE le 7 septembre 2026**, PR #274 : `verifier-environnement.sh` porte six sens sur le `.env` local, et il a trouvé **deux défauts réels dès sa première exécution**. `STRIPE_WEBHOOK_SECRET` ne concordait pas avec `stripe listen`, deux chaînes de 70 caractères au même préfixe `whsec_`, empreintes différentes : chaque événement aurait rendu 400, la commande restant en `EN_ATTENTE_PAIEMENT` alors que Stripe a encaissé. `DOCUMENTS_RACINE` était **absente**, donc aucune facture n'était ni écrite ni servie sur les trois routes qui la lisent. **La liste des variables n'est pas dans le script** : elle est dérivée de `.env.example`, qui gagne un marqueur `# @facultative <raison>`, la raison étant obligatoire. Vingt variables marquées. **Deux sens ont été imposés par la mesure**, pas par le ticket : le groupe qui va ensemble, marquer les trois variables du médiateur facultatives laissait passer un `.env` portant le nom **sans** l'adresse, ce que `lireMediateur` refuse ; et le mode `--exemple-seul`, la chaîne n'ayant pas de `.env` et n'en aura jamais, sans lequel le contrôle ne tournerait nulle part en CI. Le hook de secrets gagne le refus d'une **valeur passée en argument**, lisible par tout `ps` : il a bloqué sa propre commande de test, puis celle qui appliquait un correctif. Huit mutations sur huit, dix cas sur dix pour le hook dans les deux sens. **Quatre pièges mesurés en écrivant la preuve**, dont `bash -c` qui remplace son image et fait disparaître le secret de `ps`, cas où le contrôle restait vert **à juste titre**. **L'accès au `.env` est ouvert le même jour**, arbitrage de Christophe : les trois couches ensemble, clés privées exceptées. Un sens reste fermé, une valeur en argument étant une fuite hors du périmètre de la session. État d'avant, après la fermeture de LS-168 et de LS-201, et la création de **LS-202** : six numéros d'étape sont en doublon dans `controles.yml`, un en triple, donc « échec à l'étape 6o » désigne trois domaines sans rapport. Trouvé en ajoutant une étape pour LS-179, corrigé pour cette seule étape et ticketé pour le reste. **LS-168 LIVRÉE le 7 septembre 2026**, PR #267 : la suite de bout en bout consommait **sept inscriptions par exécution** quand `/sign-up/email` en accepte trois par minute, et la base de développement portait **118 comptes de test accumulés**, dont 68 laissés par la seule fixture vérifiée. Toutes les adresses sont désormais fixes et les comptes réutilisés : en régime établi la suite ne fait **aucun appel d'authentification**. Trois hypothèses ont été démenties par la mesure : le projet `preparation` n'était pas séquentiel, un espacement de 21 s **saturait** la fenêtre glissante de 60 s au lieu de l'éviter, et deux routes de plus étaient plafonnées, dont `/change-password` où ma propre correction réintroduisait le défaut. `verifier-fixtures-e2e.sh` entre en CI, cinq sens prouvés par mutation, dont un **angle mort du contrôle lui-même** : il cherchait un nombre écrit en clair et restait vert sur une attente calculée. Aucun plafond n'est désactivé, `src/lib/auth.ts` est inchangé. **LS-201 LIVRÉE ET CLOSE le 7 septembre 2026**, PR #269 et #270 : je l'avais ouverte sur trois tests « instables » **sans vérifier l'historique nocturne**, qui était vert. L'instabilité n'existait que sur la machine locale, à 4,48 de charge au repos. Un **défaut produit réel** s'y cachait pourtant : `revalidatePath(chemin)` n'invalide pas le layout, or c'est lui qui calcule la pastille des messages et les comptages du tableau de bord. L'exploitante classait son dernier message non lu, la liste se vidait, et la barre continuait d'annoncer « 1 ». **Six appels corrigés**, règle **C37** posée. Trois réglages successifs ont **déplacé le seuil sans rien fermer**, motif retenu en mémoire. **LS-199 LIVRÉE le 6 septembre 2026**, PR #265 : le garde-fou censé rendre visible un échec nocturne **n'avait jamais fonctionné**. `nocturne.yml` ouvre une issue à chaque échec depuis LS-177, mais l'étape échouait sur « could not add label: 'controle-nocturne' not found », l'étiquette n'ayant jamais été créée dans le dépôt. Deux nuits rouges, zéro alerte, et c'est ce silence qui a laissé les tests passer inaperçus. Le workflow crée désormais son étiquette, `--force` la rendant idempotente : **une étiquette vit dans les réglages du dépôt, pas dans le dépôt**, et ne survit ni à un clone ni à un dépôt recréé. `verifier-config-claude.sh` gagne un sens qui confronte toute étiquette posée à sa création, et il a trouvé le **même risque latent sur `derive-documentation.yml`** dès sa première exécution. Cinq tests réparés, dont **un seul réellement périmé** par une story ultérieure, celui que le ticket avait identifié : il lit désormais les entrées rendues au lieu de recopier des noms, trois ayant quitté cette liste en trois jours. Les autres relevaient de fenêtres de course ouvertes par les `loading.tsx` de LS-188, d'un marqueur ambigu satisfait par un squelette, et de deux tests exigeant du bureau ce qui n'existe qu'en mobile. **J'ai conclu à tort à un défaut produit** sur un lien qui fonctionne, en lisant l'URL figée d'un rapport d'échec. La suite passe de **9 échecs à 3**, tous relevant de LS-168. **LS-197 s'est ajoutée** depuis le relevé à quatorze du 5 septembre. **LS-137 LIVRÉE le 5 septembre**, PR #240, première story de cet epic à l'être : le socle de référencement technique, canoniques, Open Graph, JSON-LD sans quantité, sitemap engendré depuis le catalogue réel et robots.txt. Deux revues y ont trouvé deux défauts réels, **`/facture/` absent de `robots.txt`** alors que sa route jumelle `/retractation/` y figurait, elle qui sert un PDF nominatif sans session sur seule signature de jeton, et un gabarit de titre qui **doublait le nom de la boutique sur vingt-cinq pages**. `verifier-seo.sh` entre en CI, cinq mutations sur cinq détectées. **LS-150 est débloquée**, son socle technique était cette story. LS-168 ajoutée le 3 septembre sur l'instabilité de la suite de bout en bout ; **LS-169 livrée le même jour**, la chaîne ne rejoue plus les tests longs sur un diff documentaire. **Huit de ces stories attendent l'achat du VPS**, LS-151 comprise, contrairement à ce qu'annonçait le journal du 3 septembre. **LS-186 créée le 4 et LIVRÉE le 5 septembre 2026**, PR #238 : `derive-documentation.yml` déclarait `administration: read`, une portée qui n'existe pas, donc le fichier était rejeté au chargement et le contrôle hebdomadaire de dérive documentaire ne tournait plus. La signature ne ressemble pas à un échec d'étape, durée nulle et aucun job, ce qui l'a fait écarter quatre fois. `verifier-config-claude.sh` confronte désormais les portées à la liste des seize valides, prouvé par mutation. **LS-189 créée le 5 septembre 2026** : la préparation de bout en bout échoue sur une base portant un compte d'administration réel, l'index partiel n'admettant qu'une ligne et la clause `e2e-` protégeant à juste titre le compte de l'exploitante. **LS-175, LS-176 et LS-177 créées le 4 septembre.** LS-175 : rien ne créait le compte administrateur de l'exploitante, elle bloque LS-153. **LS-176 livrée**, PR #221 : la chaîne ne se rejoue plus sur `main` après fusion, `npm audit` distingue une panne du registre d'une vulnérabilité, et le navigateur de test est mis en cache ; son critère de découpage en deux jobs reste ouvert, un job sauté comptant comme un succès chez GitHub. **LS-177 livrée**, PR #223 : les scénarios de bout en bout, `npm audit` et l'image passent au contrôle nocturne, une pull request de code tombant de 24 à 14 minutes. Elle **réduit la couverture avant fusion**, arbitrage porté dans LS-153. **LS-176 et LS-177 sont closes**, la reconfiguration de la chaîne est terminée. Le contrôle nocturne a été vert à sa première exécution, puis **rouge deux nuits sans alerter personne**, ce que LS-199 a corrigé. **LS-178 créée puis CLOSE le même jour** : `npm ci` était devenu le premier poste de dépense, de 14 à 431 s sur le même verrou. La cause est **externe**, l'écriture de 1,3 Go de `node_modules` à environ 3,2 Mo/s sur le disque de l'exécuteur, et non les scripts d'installation que cinq hypothèses successives ont accusés à tort **ADR-036 ACCEPTÉ le 8 septembre 2026**, session g : la boutique est hébergée sur le **VPS déjà en service pour SmartPlanning**, en cohabitation, aucun VPS n'étant achetable. Le catalogue OVHcloud était **intégralement en rupture**, quatorze localisations européennes et gamme précédente comprises, Hetzner présentant le même symptôme le même jour ; laissé à son défaut, le configurateur plaçait le serveur en **Amérique du Nord**. La machine a été **mesurée et non supposée** : 6,4 Go de mémoire et 64 Go de disque libres, charge nulle, soit le VPS-2 envisagé occupé à 15 %. Trois tickets affirmaient depuis des semaines qu'aucune machine n'existait, en recopiant l'hypothèse sans jamais la mesurer. **LS-151 EN COURS, sept critères sur huit**, session h, PR #315 : `https://lune-soleil.fr` répond en TLS, certificat jusqu'au 7 décembre 2026, **sans que SmartPlanning ait été interrompu**. Trois pièges désamorcés par la documentation de l'autre projet : `http2 on;` n'existe qu'à partir de Nginx 1.25 et la machine sert la 1.24, les zones `limit_req_zone` vivent dans le contexte `http` **global** donc un nom réutilisé casserait les trois sites, et le `nginx.conf` global autorise TLSv1 et TLSv1.1 dont la boutique aurait **hérité en silence**. `verifier-nginx.sh` a refusé ma première version du challenge ACME, `root` et `alias` étant interdits, LS-132 critère 6 : **c'est l'usage qui a cédé**, pas la règle, et le challenge passe par certbot en `standalone`. **Le critère 2 échoue** : les ports 3000 et 3001 de SmartPlanning étaient joignables depuis Internet, **Docker insérant ses règles DNAT en amont d'`ufw`**, ce que `ufw status` ne montre pas et que seule une mesure depuis l'extérieur révèle. Défaut d'un autre projet, corrigé par Christophe le jour même, SP-583, et vérifié fermé. La story se ferme quand LS-152 aura posé l'application et que le port 3002 sera vérifié non joignable |
...
**139 tickets terminés sur 192, soit 72 %**, relevés dans Jira le 7 septembre
2026 après la fermeture de LS-168, LS-201, LS-179, LS-156, puis LS-143, LS-161,
LS-202, LS-9, LS-198 et LS-197.
...
État d'avant, relevé le 5 septembre 2026 : **115 tickets terminés sur 180, soit
64 %**, après la
