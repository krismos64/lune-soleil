...
| `src/services/purge-journaux.ts` | les **cinq** purges de rétention, tâche quotidienne, LS-94 et LS-154 | **une purge par âge seul est fausse dès qu'une table porte des états** : `ENVOI_EN_COURS` est un incident à traiter, la purger l'effacerait en silence. Le filtre est une **liste positive**, une négation laisserait purger tout statut ajouté à l'enum. Les frontières sont en `lt` strict, et le test s'ancre sur la fonction du code plutôt que de recopier le calcul |
