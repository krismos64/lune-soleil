...
| `docs/PREUVES-PAR-MUTATION.md` | **la couverture des preuves par mutation**, LS-230 : où chaque preuve est **déclarée**, et le motif écrit de celle qui ne tourne pas. Déclarée n'est ni « prouve encore » ni « s'exécute » : un `bash -e` a masqué quatre preuves sur six trois nuits, LS-233, et le symptôme se lit en **comptant les bilans** du rapport. Le compte se mesure et ne se lit pas, `ls scripts/*-mutation.sh | wc -l` valait **44** le 15 septembre 2026, **38 par PR et 6 au nocturne**. Une exemption sans motif est un interrupteur, pas une décision | écrire ou modifier une preuve par mutation, ajouter une étape de CI, **grouper plusieurs scripts dans un même step** |
