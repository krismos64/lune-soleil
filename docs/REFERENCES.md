...
| `src/integrations/email/modeles.ts` | rendu des trois messages d'authentification, texte brut | **les six textes F-MAIL-01 à F-MAIL-06 restent dus par LS-29**, ils demandent la validation de l'exploitante et n'entrent pas ici sans elle. **La variable s'appelle `lien`** : `auth.ts` la nommait `url`, donc `rendreModele` levait et aucun email d'authentification ne partait, six semaines durant. Chaque moitié était testée séparément et correctement, LS-54 |
