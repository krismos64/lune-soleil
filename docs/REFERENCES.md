...
| `src/integrations/email/modeles.ts` | rendu de tous les messages, texte brut | **les textes sont écrits depuis LS-29**, 12 septembre 2026, ton validé par l'exploitante : signature « L'atelier Lune-soleil », vouvoiement, « article » et jamais « bijou ». Le compte de modèles ne s'écrit pas ici, il se mesure sur le type `ModeleEmail`. **Aucune version HTML n'est emise**, LS-222 la porte, et le texte devra y rester en repli. **La variable s'appelle `lien`** : `auth.ts` la nommait `url`, donc `rendreModele` levait et aucun email d'authentification ne partait, six semaines durant. Chaque moitié était testée séparément et correctement, LS-54 |
