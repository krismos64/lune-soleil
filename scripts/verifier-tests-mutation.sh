...
#
# IL A ATTRAPE UNE OMISSION REELLE LE 22 SEPTEMBRE 2026, LS-235.
# `$URLS_MEDIAS` etait declare et mute plus bas, mais absent d'ici : il
# n'aurait ete ni sauvegarde ni restaure par copie, et le filet `git checkout`
# de `nettoyer` ne parcourt que cette liste. Une interruption au mauvais moment
# laissait donc `src/integrations/medias/urls.ts` MUTE sur le disque, le meme
# motif qui avait laisse `auth.ts` sans sa garde `input: false` en LS-230.
#
